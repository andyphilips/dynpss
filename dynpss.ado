...
capture program drop dynpss
capture program define dynpss , rclass
...
qui forv i = 1/`n_vars'	{			// for every var
	...
	* for the lags  -----
	if "`lags'" != ""	{
		loc lagnum : word `i' of `lags'	// count of lags
		cap drop L`r'_`varname_`i''
		if "`i'" == "1"	{					// take care of LDV first
			if "`lagnum'" == "."	{
				di in r _n "At least 1 lag of the dependent variable must be specified"
				exit 198
			}
			else	{
				loc totldvs `lagnum'		// need # of LDVs for later
				forv r = 1/`lagnum'	{		// for all lags of the DV
					gen L`r'_`varname_`i'' = l`r'.`varname_`i'' // create up to r lags of DV as needed
					loc ldvs "`ldvs' L`r'_`varname_`i''"	// add to ldvs
				}		// close LDV loop
			}			// close else
		}				// close for DV
		else if "`varname_`i''" == "`shockvar'"	{	// if lagged shockvar
			if "`lagnum'" == "."	{
			}
			else	{
				forv r = 1/`lagnum'	{
					gen L`r'_`varname_`i'' = l`r'.`varname_`i'' // gen lagged vars as needed
					loc lsiv "`lsiv' L`r'_`varname_`i''" // add to lag shockset
					loc totlagshocks `r' // counter of total lagged shocks for below
				}
			}
		}
			...
			if "`lagnum'" == "."	{
				...
				forv r = 1/`lagnum'	{							
					...
					loc livset "`livset' L`r'_`varname_`i''"
				}
			}	
		}
	}							// close if lags != .
	...
	* For the lagged diffs -----
	...
	if "`lagdiffs'" != ""	{					
		...
		cap drop L`lagdiffnum'D_`varname_`i''
		if "`i'" == "1"	{						// take care of DLDV first
			if "`lagdiffnum'" == "."	{
			}
			else	{
				gen L`lagdiffnum'D_`varname_`i'' = l`lagdiffnum'd.`varname_`i'' // generate lag-diff
				loc lddvs "`lddvs' L`lagdiffnum'D_`varname_`i''"
				loc lagdiffnum_dv = `lagdiffnum' // need for below
			}	
		}
		else if "`varname_`i''" == "`shockvar'"	{ // then the ld shock var
			if "`lagdiffnum'" == "."	{
			}
			else	{
				gen L`lagdiffnum'D_`varname_`i'' = l`lagdiffnum'd.`varname_`i''
				loc ldsiv "`ldsiv' L`lagdiffnum'D_`varname_`i''"
				loc lagdiffnum_siv = `lagdiffnum' // need for below
			}
		}
		else	{					// add everything else to other varlist
			if "`lagdiffnum'" == "."	{
			}
			else	{
				gen L`lagdiffnum'D_`varname_`i'' = l`lagdiffnum'd.`varname_`i''
				loc ldivset "`ldivset' L`lagdiffnum'D_`varname_`i''"
			}
		}
	}							// close if lagdiffs != .
...
if "`level'" != "" & "`ec'" != ""	{	// issue var appearing in levels in ECM warning
...
* create parsing routine that moves through regression varlist (and set), 
...
* now multiply [s x k] posterior beta matrix by [k x 1] set matrix
tempname PV
mat `PV' = PB*set	
svmat `PV'
...
qui forv p = 2/`brange'	{	
	...
	* Set ldv's to predicted value:
	...
	foreach var in `ldvs'	{	// ldv's are ordered L1.Y L2.Y ...
		if `w' > 0	{			// if previous LDV values exist. If not, it remains at sample mean until enough t have occurred.
			mat set[`row',1] = meanpv_`w'
		}
		else	{
		}
		local w = `w' - 1		// move back 1 for every l.Y var.
		loc row = `row' + 1		// move down set list
	}
	
	* set lagged diff Y's to predicted changes lagged t - (t-1), t-1 - (t-2)...:
	foreach var in `lddvs'	{
		loc w = `p' - `lagdiffnum_dv' // subtract current time by whatever ld.Y is
		if `w' > 1	{
			loc wm1 = `w' - 1
			mat set[`row',1] = meanpv_`w' - meanpv_`wm1'  // lagged diff set matrix replaced with value of the difference at w'th lag
		}
		else	{
		}
		local row = `row' + 1
	...
	* else if t > shocktime ------------------------------
		...
		* lagged var shock:
		...
		foreach var in `lsiv'	{
			gettoken dynamics name: var, parse("_")
			if `w' >= `lshocks'	{		// lag only starts to feel shock after a lag
				mat set[`row',1] = `name'_mean + `shockval'
			}
			else	{					// lag remains unchanged
			}
			local lshocks = `lshocks' + 1	// move back 1 for every l.shockvar.
			local row = `row' + 1
		}
		...
		* lagged diff var shock:
		...
		/* shock in levels:
		* shock var already shocked! So don't need to do anything.
		*/
...
if "`saving'" != "" {
	...
	noi save dynpss_results.dta, replace
}
